#include "Test3DLightMap_OpenGL.h"
...
namespace test {
	...
	Test3DLightMap_OpenGL::Test3DLightMap_OpenGL(GLFWwindow* window)
...

	{
		...
		ObjLoader::loadObjModel("res/models/BakeScene_full.obj", m_vertices, m_indData);
		...
		m_Texture = std::make_unique<Texture>("res/textures/BakeScene_full.png");
		m_Texture->Bind(0);
		...
		//			LIGHT MAP
			...
			// Эффект Питера Пена - glCullFace(GL_FRONT) при рисовании карты теней, glCullFace(GL_FRONT) при рендеринге сцены 
		// -----------------------------------
	...
	Test3DLightMap_OpenGL::~Test3DLightMap_OpenGL()
	{
		m_window = nullptr;
		// Не нужно ничего удалять, т.к. используем умные указатели
	}

	void Test3DLightMap_OpenGL::OnUpdate(float deltaTime)
	{		
		computeMatricesFromInputs(deltaTime);
	}

	void Test3DLightMap_OpenGL::OnRenderer()
	{
		Renderer renderer;
		renderer.Clear();
		int wWidth, wHeight;
	...
	void Test3DLightMap_OpenGL::OnImGuiRenderer()
	{
		
	}

	void Test3DLightMap_OpenGL::computeMatricesFromInputs(float deltaTime)
	{
		//					ORIENTATION
		double xpos, ypos;
